...
class Points(commands.Cog):
    ...
    async def add_points(self, interaction: discord.Interaction, amount: int, users: str):
        ...
        if users:
            ...
            data = ""
            counter = 1
            await interaction.response.send_message(embed=discord.Embed(description="{} Adding points to {} users.".format(config.LOAD_EMOJI, len(matches)), color=config.TFC_GOLD), ephemeral=True)
    ...
    async def remove_points(self, interaction: discord.Interaction, amount: int, users: str):
        ...
        if users:
            ...
            for match in matches:
                ...
                await interaction.response.send_message(embed=discord.Embed(description="**Removed `{}` points from the following users:**\n{}".format(amount, data), color=config.TFC_GOLD), ephemeral=True)
    ...
    async def check_points(self, interaction: discord.Interaction, user: discord.Member=None):
        ...
        if interaction.channel == bot_commands_channel or bot_operator_role in interaction.user.roles:
            ...
            await interaction.response.send_message(embed=discord.Embed(description="{} have {} {} points.".format(possesive_pronoun, config.POINTS_EMOJI, points), color=config.TFC_GOLD), ephemeral=True)
    ...
    async def leaderboard(self, interaction: discord.Interaction):
        ...
        if interaction.channel == bot_commands_channel or bot_operator_role in interaction.user.roles:
            ...
            if data is None or data == []:
                await interaction.response.send_message(embed=discord.Embed(description="{} No leaderboard data found.".format(config.ERROR_EMOJI), color=config.TFC_GOLD), ephemeral=True)
                return
            
            await interaction.response.send_message(embed=discord.Embed(description="{} Fetching leaderboard data...".format(config.LOAD_EMOJI), color=config.TFC_GOLD), ephemeral=True)
            formatted_data = ""
            counter = 1
            for entry in data:
